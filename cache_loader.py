...
class MyDataset(Dataset):
    ...
    def __init__(self, cfig, phase):
        ...
        for data_path in self.data_list:
            ...
            data_npz = np.load(data_path, allow_pickle=True)  # about 200 GB memory usage
            # In_dict = dict(data_npz)['arr_0'].item()
            self.data_npz.append({'data_path': data_path, 
                                  'ID': ID,  
                                  'PatientID': PatientID,
                                  'data_npz': data_npz,
                                #   'In_dict': In_dict # fixme why signifiacntly increase the memory usage
                                  })
    ...
    def __getitem__(self, index):
        ...
        data_path = self.data_npz[index]['data_path']
        # ID = self.data_list[index].split('/')[-1].replace('.npz', '')
        # PatientID = ID.split('+')[0]
        ID = self.data_npz[index]['ID']
        PatientID = self.data_npz[index]['PatientID']

        # if len(str(PatientID)) < 3:
            # PatientID = f"{PatientID:0>3}"
        
        # data_npz = np.load(data_path, allow_pickle=True)
        data_npz = self.data_npz[index]['data_npz']

        In_dict = dict(data_npz)['arr_0'].item()
        # In_dict = self.data_npz[index]['In_dict']
